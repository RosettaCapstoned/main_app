      ...
      <div>
        <div className="chat-room">
          <AppBar
            position="static"
            align="center"
            style={{ backgroundColor: '#FD9764' }}
          >
            <Typography
              variant="h4"
              color="textPrimary"
              style={{ fontFamily: 'Georgia, serif', opacity: 0.8 }}
            >
              rcsCHAT
            </Typography>
          </AppBar>
            ...
            <ul className="list-unstyled chat" id="chatList">
              {messages &&
                messages.map((each, idx) => {
                  ...
                  if (
                    each.message === '' ||
                    each.message === undefined ||
                    each.message === null ||
                    each.message.message === undefined
                  )
                    return null;
                  return (
                    <li
                    ...
                    >
                      <Avatar className={avatar}>
                        {
                          /*each.name ? each.name[0] || 'R' : each.message.name[0] ||*/ 'R'
                        }
                      </Avatar>
                      {each.name ? (
                        <div className="chat-body pull-left card primary p-3 ml-2 z-depth-1">
                          <p className="mb-0">{each.message || ''}</p>
                        </div>
                      ) : (
                        <div className="chat-body pull-left card primary p-3 ml-2 z-depth-1">
                          <p className="mb-0">{each.message.message || ''}</p>
                        </div>
                      )}
                    </li>
                  );
        ...
        >
          ...
          <div className="chatButtonContainer">
            <IconButton onClick={handleClick}>
              <Icon style={{ color: '#FD9764' }}>send</Icon>
            </IconButton>
          </div>
        </div>
      </div>
